...
function request ($url) {
	$request = http_request (str_replace (' ', '%20', $url));
	if ($request->code == 200) {
		return $request->data;
	} else {
		print "could not load from $url, request code: $request->code";
		die;
	}
}

function http_post_data ($url, $content) {
	$ch = curl_init();
...
function mustache ($template, $string) {
  $m = new Mustache;
  return $m->render($template, $string);
}


// YAML cheat functions

function yaml_write ($array) {
    return Spyc::YAMLDump($array);
}

function yaml_read ($string) {
    return Spyc::YAMLLoad($string);
}

function yaml_ctn ($array) {
  $string = yaml_write($array);
  return ltrim ($string, "---\n");
}
...
function directory_txts_non_recursive ($path, $result = array()){
  $dir = scandir($path);
  foreach($dir as $f){
		if (substr($f, -4, 4) == '.txt' and !is_hidden($f)) {
      $result[$path."/".$f] = $path."/".$f;
		}
  }
  return $result;
}
...
function extract_name ($_str) {
  $str1 = array_pop(explode("/", $_str));
  if (in_str ('.', $str1)) {
    $file_parts = explode (".", $str1);
...
function is_img ($name) {
	$str = strtolower($name);
	$is = false;
	$suffixes = array ('.jpg', '.jpeg', '.png', '.gif', '.img');
	foreach ($suffixes as $suffix) {
		if (in_str($suffix, $str)) $is = true;
	}
	return $is;
}
...
function is_file_by_suffix ($name) {
	$str = strtolower($name);
	$is = (is_img($str)) ? true : false;
	$suffixes = array ('txt');
	$last = array_pop(explode('.', $name));
	foreach ($suffixes as $suffix) {
		if ($last == $suffix) $is = true;
	}
	return $is;
}
